...
function CBLF(img,σs,σr,guide=nothing,τ=0.005)
    ...
    T = (low+up)/2
    if size(channelview(img), 1) != size(img, 1)
        b0 = [RGB(1.0, 1.0, 1.0) for i in 1:size(img, 1), j in 1:size(img, 2)] #ones(eltype(img), size(img))
    else
        b0 = [Gray(1.0) for i in 1:size(img, 1), j in 1:size(img, 2)] #ones(eltype(img), size(img))
    end
    b = O1gaussnd(img,σs)
    @inbounds for k in 1:K
        ω = 2*π*k/T
        ak = 2*exp(-0.5*ω*ω*σr*σr)
        c = mapc.(x -> cos(ω*x), guide) #cos.(ω.*guide)
        s = mapc.(x -> sin(ω*x), guide) #sin.(ω.*guide)
        Ψc = O1gaussnd(c,σs)
        Ψs = O1gaussnd(s,σs)
        b0 .+= ak.*(mulimg(c, Ψc).+mulimg(s, Ψs)) #ak.*(c.*Ψc.+s.*Ψs)
        Ψc = O1gaussnd(mulimg(c,img),σs) #O1gaussnd(c.*img,σs)
        Ψs = O1gaussnd(mulimg(s,img),σs) #O1gaussnd(s.*img,σs)
        b .+= ak.*(mulimg(c, Ψc).+mulimg(s, Ψs)) #ak.*(c.*Ψc.+s.*Ψs)
    end
    return divimg(b, b0) #b./b0
end

# function for point wise multiplication of images
function mulimg(a, b)
    (all(size(a) .== size(b)) ? nothing : throw(ArgumentError("images should be of the same size")))
    if size(channelview(a), 1) != size(a, 1)
        out = similar(a,eltype(a),size(a))
        for i in CartesianIndices(a)
            out[i] = RGB(a[i].r * b[i].r, a[i].g * b[i].g, a[i].b * b[i].b)
        end
    else
        out = similar(a,eltype(a),size(a))
        for i in CartesianIndices(a)
            out[i] = a[i].val * b[i].val
        end
    end
    return out
end

# function for point wise division of images
function divimg(a, b)
    (all(size(a) .== size(b)) ? nothing : throw(ArgumentError("images should be of the same size")))
    if size(channelview(a), 1) != size(a, 1)
        out = similar(a,eltype(a),size(a))
